...
const FT_PRICING = { female: 299, male: 399 };
...
class BookingPopup{
    ...
    showSuccess(result){
        ...
        document.getElementById('b_sumQty').textContent = result.quantity != null ? result.quantity : '—';
        // amount is stored in INR (e.g. 299), never in paise — show as-is with fallback
        const amt = result.amount != null ? Number(result.amount) : null;
        document.getElementById('b_sumPrice').textContent = amt != null ? `₹${amt.toLocaleString('en-IN')}` : '—';
